...
void luuMang(int n, int vitri) {
  fstream outFile;
  outFile.open("MangSoNgauNhien.txt", ios::out);
  outFile << (n - vitri) << " ";
  for (int i = vitri; i < n; i++) {
    outFile << mangMaDocGia[i] << " ";
  }
}
...
void Introduce() {
  ...
  if (inFile.good()) {
    while (!inFile.eof()) {
	  ...
	  // Tra ve loi
    cout << "\n"
         << "Lien ket voi File introduce khong thanh cong! "
         << "\n";
  }

  SetColor(LIGHT_GREEN);
  gotoxy(xTen + 10, yTen);
  cout << "HOTEN : " << MANAGER_USER_NAME_1;
  gotoxy(xTen + 10, yTen + 1);
  cout << "MSSV  : " << MANAGER_USER_ID_1;
  gotoxy(xTen + 10, yTen + 2);
  cout << "LOP   : " << MANAGER_USER_DEPARTMENT;

  gotoxy(25, yTen);
  cout << "HOTEN : " << MANAGER_USER_NAME_2;
  gotoxy(25, yTen + 1);
  cout << "MSSV  : " << MANAGER_USER_ID_2;
  gotoxy(25, yTen + 2);
  cout << "LOP   : " << MANAGER_USER_DEPARTMENT;
...
void ManHinhChinh() {
  ...
  {
    cout << "\n"
         << "Lien ket voi File QuanlyThuVien khong thanh cong! "
         << "\n";
  }
...
void Save(TREE_DG t, fstream &file) {
  ...
  file << t->dsmt.n << endl;
  for (NODE_MT p = t->dsmt.pHead; p != NULL; p = p->right) {
...
void Load_DS(LIST_DAUSACH &lDS) {
  ...
  if (inFile.is_open()) {
    ...
    for (int i = 0; i < sodausach; i++) {
      ...
      //			gotoxy(0,0);
      //			cout << "So luong: " << pDS->soluongsachdamuon;
      //			_getch();
    }
  } else {
    cout << "KET NOI VOI FILE DocGia THAT BAI! ";
...
bool KiemTraDMS(LIST_DMS lDMS) {
  for (NODE_DMS *p = lDMS.pHead; p != NULL; p = p->pNext) {
    if (p->data.ttSach == 0) return true;
  }
  return false;
}
...
int Menu_MuonSach(LIST_DAUSACH &lDS, NODE_DG *nDG) {
  ...
  bool check;
  char thongbao1[] =
      "            Sach da co DOC GIA muon. Moi ban chon cuon sach khac!";
  char thongbao2[] =
      "              Sach da duoc THANH LY. Moi ban chon cuon sach khac!";
  char thongbao3[] =
      "                 DOC GIA da muon sach thuoc Dau Sach nay!     ";
  char thongbao4[] =
      "					Sach trong dau sach nay da duoc "
      "muon het !!!";
  normalBGColor();
...
label:
  do {
    ...
    // lay ma isbn
    string isbn = pDS->info.ISBN;
    check = SearchISBN_MT(nDG->dsmt, isbn);

    // check = SearchTenSach_MT(nDG->dsmt, tensach);
...
label1:
  ...
  do {
    ...
    if (choose2 == -1) {
      ...
      // xem th? ch? d?u s�ch c� quay l?i ngay c�i trang h?u n?y lu�n kko
      ...
      // trang thai sach = 1, sach da co nguoi muon, nen se khong muon sach nay
      // duoc.
      ...
      // trang thai sach = 0 cho muon duoc, nen se tien hanh muon sach.
        ...
        // nhap thong tin ngay muon va xu ly truong hop nhap ngay muon lon hon
        // ngay he thong;
        do {
          gotoxy(83, 28);
          ...
          if (check3 == -1) {
            ...
            // nap thong tin vao cau truc muon tra bao gom ten sach, ngay muon,
            // ngay tra..v.v.v
            //				mt.tensach = tensach;
            mt.ngaytra.nam = 0;
            mt.ngaytra.thang = 0;
            ...
            // lay ma sach va vi tri sach... cap nhap trang thai sach da duoc
            // muon.

            AddHeadList_MT(nDG->dsmt, mt);
            // goto label1;
            return 1;
          }
        } while (check3);
...
void XuLyMuonSach(TREE_DG &t, LIST_DAUSACH &lDS) {
  ...
  char thongbao2[] =
      "           DOC GIA bi khoa the nen khong duoc muon sach !";
  char thongbao3[] =
      "         DOC GIA khong duoc muon sach vi da giu sach qua han 7 ngay!";
  char thongbao4[] =
      "         DOC GIA khong duoc muon sach vi da lam mat sach ^_^ ";
...
label:
  ...
  if (check1 == -1) {
    ...
    label1:
      ...
      gotoxy(93, 14);
      cout << "- So sach  da muon : " << temp->dsmt.n;
      gotoxy(93, 15);
      cout << "- So sach chua tra : " << SoSachDangMuon(temp->dsmt);
      ...
      cout << "_Ngay Muon__";
      int i = 0;
      for (NODE_MT p = temp->dsmt.pHead; p != NULL; p = p->right) {
        // truong hop dang muon chua tra, va truong hop dang muon nhung bi mat
        // sach.
...
void XuLyTraSach(TREE_DG &t, LIST_DAUSACH &lDS) {
  ...
  char thongbao1[] =
      "    Thong tin da duoc cap nhat. Bao mat sach thanh cong !";
  int check, check1 = 0;
  NODE_DG *temp;
  NGAY_THANG ngaytra;
...
label:
  ...
  if (check == -1) {
    ...
    label1:
      ...
      DisplayMT(keyDisplayMT, 6, XDisplayMT);
      OutputList_MT(temp->dsmt, lDS);
      ...
      do {
        while (_kbhit()) {
          ...
          switch (kb_hit) {
            case KEY_F3:
              Beep(600, 100);
              choose1 = chonItem_MT(temp->dsmt, lDS);
              if (choose1 == -1) {
                chuChay(thongbao, 50, 30);
                return;
              }
              i1 = -1;
              for (NODE_MT pMT = temp->dsmt.pHead; pMT != NULL;
                   pMT = pMT->right) {
                if (pMT->data.trangthai == 0 || pMT->data.trangthai == 2) {
                  ...
                  if (i1 == choose1) {
                    ...
                    do {
                      ...
                      // gotoxy(0, 0);
                      cout
                          << "Nhap vao ngay tra. Nhan F10 de luu, ESC de huy !";
                      ...
                      if (check1 == 2) {
                        if (SoSanhNgay(pMT->data.ngaymuon, ngaytra) < 0) {
                          ...
                          string ma = pMT->data.MASACH;
                          string isbn = ma.substr(0, 6);

                          pDAU_SACH pDS = SearchTen_TSBN(lDS, isbn);

                          // pDAU_SACH pDS =
                          // SearchTen_DS(lDS,pMT->data.tensach);
                          // tim kiem sach co ma dau sach ma doc gia da muon.
                          NODE_DMS *pDMS = Search_DMS1(pDS, pMT->data.MASACH);

                          // thay doi trang thai sach tu da co nguoi muon sang
                          // chua co nguoi muon
                          pDMS->data.ttSach = 0;
                          pDS->soluongsachdamuon--;
                          goto label1;
            ...
            case KEY_F4:
              Beep(600, 100);
              choose2 = chonItem_MT(temp->dsmt, lDS);
              if (choose2 == -1) {
                chuChay(thongbao, 50, 30);
                return;
              }
  ...
  label:
    ...
    switch (type) {
      case 0:
        ...
        // label:
        result =
            ChooseSubMenu(keySubMainMenu1, xSubMainMenus1, ySubMainMenus1, 3);
        if (result == -1) {
          check = true;
          goto label;
        }
        if (result == 0) {
          Menu_DocGia(t, mangMaDocGia);
        } else if (result == 1) {
          InDocGia(t);
          //	goto label;
        } else if (result == 2) {
      ...
      case 1:
        ...
        result =
            ChooseSubMenu(keySubMainMenu2, xSubMainMenus2, ySubMainMenus2, 3);
        if (result == -1) {
          check = true;
          goto label;
        }
      ...
      case 2:
        ...
        result =
            ChooseSubMenu(keySubMainMenu3, xSubMainMenus3, ySubMainMenus3, 3);
        if (result == -1) {
          check = true;
          goto label;
        }
